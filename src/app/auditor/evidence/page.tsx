  ...
  return (
    <Shell user={user}>
      ...
      <p className="mb-6 mt-2 max-w-2xl text-sm text-[color:var(--muted)]">
        Every document ingested across every application, with its evidence-trust level (A–E) and OCR confidence —
        separate signals, per the source-document trust model. {lowTrust} document{lowTrust === 1 ? "" : "s"} sit at
        trust D/E and rely on a human confirmation rather than a trusted source.
      </p>
          ...
          <tbody>
            {rows.map(({ app, doc }) => (
              <tr key={doc.id} className="border-t border-[color:var(--border)]">
                ...
                <td className="meta">{doc.quality}</td>
              </tr>
            ))}
          </tbody>
        </table>

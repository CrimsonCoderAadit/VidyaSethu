      ...
      {db.aiFeedback.length ? (
        <section className="card mb-6 p-5">
          ...
          <p className="mb-3 text-sm text-[color:var(--muted)]">
            Every officer correction to an AI-extracted field, with model version — a curated dataset, not a live
            retraining feed.
          </p>
          <ul className="space-y-3 text-sm">
            {db.aiFeedback.map((f) => (
              <li key={f.id} className="border-t border-[color:var(--border)] pt-3">
                ...
                <p className="meta">{f.at} · {f.reason}</p>
              </li>
            ))}
          </ul>
        </section>
          ...
          <ul className="space-y-2 text-sm">
            {db.renewals.map((r) => (
              <li key={r.id} className="border-t border-[color:var(--border)] pt-2 first:border-0 first:pt-0">
                {r.applicationId} · {r.status} {r.note ? `— ${r.note}` : ""}
              </li>
            ))}
          </ul>
        </section>
